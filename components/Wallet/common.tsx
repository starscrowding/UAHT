      ...
      {action === 'output' ? (
        <div>
          {balance >= MINIMUM ? (
            ...
            <div>Мінімальний вивід {MINIMUM}. Поповни баланс та спробуй ще раз 🤑</div>
          )}
        </div>
      ) : null}
    </div>
...
export const VerificationModal = ({vModal, setVModal}: any) => {
  return (
    ...
    >
      <Modal.Header>
        <Text size={18}>Код успішно згенеровано ✅</Text>
      </Modal.Header>
      <Modal.Body>
...
export const Tips = ({
  ...
  disabled = false,
  helperText = 'чай',
  infoText = '👌 чай - це винагорода відносно запиту, компенсує газ та визначає пріоритет обробки ⌛',
}: any) => {
  return (
    <>
